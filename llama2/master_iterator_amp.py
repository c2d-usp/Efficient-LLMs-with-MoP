...
from prune_groups_amp import prune_groups
...
def main(args):
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if decision == 'L':
            params_before, params_after = prune_layers(args, iteration, args.save_dir, tune_eval=False)
            pruned_type = "LAYER"
            print(f'ITERATION {iteration} PRUNED LAYER')
        elif decision == 'G':
            params_before, params_after = prune_groups(args, iteration, args.save_dir, tune_eval=False)
            pruned_type = "GROUP"
...
def print_and_save_summary(decision_sequence, save_dir, initial_params, params_after_iterations):
    # Determine number of iterations from the data
    ...
    for i in range(num_iterations):
        ...
        # Calculate compression for this iteration
