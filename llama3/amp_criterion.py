...
# LLaMA3 specific settings for Grouped Query Attention
LLAMA3_SETTINGS = {
    "head_dim": 128,
    "group_size": 4,  # For GQA: group query heads in groups of 4
    "attn_proj": "o_proj",
    "mlp_proj": "down_proj"
}


def is_llama3_model(model):
    """Check if model is Llama 3 based on config"""
    return hasattr(model.config, 'num_key_value_heads') and model.config.num_key_value_heads != model.config.num_attention_heads
